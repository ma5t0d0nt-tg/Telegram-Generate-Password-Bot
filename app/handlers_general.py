...
async def handler(message: Message):
    ...
    if status == "1":
        await message.reply(text=f"Здравствуйте, {message.from_user.first_name}.\n"
                                 ...
                                 f"регистрации на сайтах и сторонних сервисах.\n_Сгенерированные пароли никуда не "
                                 f"сохраняются._", parse_mode=ParseMode.MARKDOWN)
...
async def handler(message: Message):
    ...
    if status == "1":
        example_tmp = (f"💠Примеры **правильных** шаблонов:\n"
                       ...
                       f"заглавных, прописных, цифр и спец. символов. Таких паролей будет сгенерировано - 10 штук.\n"
                       f"👉`pass l5 AZ az -09 -!@ n15` - сгенерирует пароль длиной 5 символов из заглавных и прописных."
                       f"Таких паролей будет сгенерировано - 15 штук.\n\n"
                       f"💠Примеры **неправильных** шаблонов:\n"
                       f"👉`pass l8 -AZ -az -09 -!@ n10` - при задании такого шаблона, бот выдаст ошибку, "
                       f"потому что все параметры исключены из генерации пароля.\n"
                       f"👉`pass lr -uZ -a7 -9 - n10=` - такой шаблон не правильный:\n"
                       f"вместо длины, стоит буква;\n"
                       f"если нужно указать, чтобы присутствовали или отсутствовали заглавные символы, нужно указывать "
                       f"'AZ' или '-AZ';\n"
                       f"с прописными символами аналогично заглавным;\n"
                       f"если нужно указать, что нужны или не нужны цифры в генерации, то шаблон должен содержать "
                       f"'09' или '-09', для параметра должны присутствовать обе цифры;\n"
                       f"вместо параметра не может стоять прочерк;\n"
                       f"Неправильные шаблоны не выдадут результат.\n\n"
                       f"‼️‼️***ПРИМЕЧАНИЕ***‼️‼️\n"
                       f"Созданный пароль можно скопировать нажатием на этот пароль. Он добавится в буфер обмена.\n"
